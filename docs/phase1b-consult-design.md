...
| 2026-08-31 | Model-policy CI runtime compatibility correction | The first approved push reached Vercel Preview and began the ACR build, but CI stopped before application build because Node 22.22.2 does not accept the later `--test-isolation=none` spelling. The suite still needs in-process execution in the restricted local runner, so only `test:model-policy` was changed to Node 22's compatible `--experimental-test-isolation=none` alias; the six policy cases remain unchanged and pass locally. No Azure revision, index, flag, request, or traffic change occurred. |
