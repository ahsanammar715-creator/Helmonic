...
| `scripts/corpus_audit/build_corpus_pilot_payload.py` | Deterministic PDF-batch coordinator with one bounded child process per source, atomic per-source checkpoints, hash-verified resume, fail-closed manual-review reporting, and an explicit pagefile-aware exclusive lane for approved per-source memory retries |
...
| `scripts/corpus_audit/run-corpus-batch-917.ps1` | Approved one-worker launcher for 900 untouched PDFs plus the seventeen prior quarantines, restricting the fifteen memory retries to an exclusive 4 GiB/ten-minute pagefile-aware lane |
| `scripts/corpus_audit/test_corpus_pilot_payload.py` | Offline selection and hardening tests covering exclusions, two-reader outcomes, worker timeout, worker memory termination, checkpoint mismatch rejection, and exclusive retry-lane physical/commit gates |
...
Drawing and floor-plan source probes must be generated from structured drawing metadata,
using the drawing title and project title where present. They must not be assembled from
the first distinct words in spatially extracted drawing text: legends, room labels, and
annotations are not a realistic user question and have now caused two false failures.
Known-answer overrides remain valid where a consultant-meaningful question has already
been proven. Otherwise the general generator emits a natural title-derived document
request; non-drawing documents retain the deterministic text fallback. This is standing
validation policy for every future batch. A failed drawing probe is still investigated
rather than silently waived, but the corrected generator prevents the known test-design
...
| 2026-09-08 | D-035 500-PDF local gate passed; Azure completion blocked after the approved retry | The bounded one-worker local run attempted exactly 500 deterministic PDFs and completed in 2h17m11s: 401 passed directly, 82 passed repair verification, and 17 were honestly quarantined without OCR, invented content, or automatic backfill. The 483 accepted originals total 1,093,332,836 bytes across 2,984 pages, 4,467 evidence chunks, and 898 table pages. The first candidate-only Azure execution verified and retained all 483 originals in Blob, then failed before any Search write because one table input exceeded the embedding model's 8,192-token limit. Commit `64b268f` added a lossless 7,000-UTF-8-byte embedding-input ceiling and weighted normalized recombination; the real payload proves 4,467 original chunks become 4,514 safe model inputs across 31 segmented chunks with exact text preservation. The one approved same-scope retry then failed before embeddings because Azure returned `409 BlobAlreadyExists` while the idempotency path recognized only `412`; commit `7b6123e` now accepts only `412` or `409` with `x-ms-error-code=BlobAlreadyExists`, followed by the existing mandatory length/hash/metadata HEAD verification. No third execution was started. After each terminal execution, embedding capacity was restored from 100 to 1 kTPM and verified. The disposable job is absent; temporary ACR repository `helmonic-consult-corpus-batch-500`, both tags/manifests, and repository-scoped cleanup assignment `8ab760d1-7241-48c6-b422-b41f53c50b5a` were deleted and independently verified absent. Candidate Blob originals and isolated candidate index remain; the index has no batch Search documents because both failures preceded the upload phase. Live `consult-demo-v2`, its app setting, and 100% traffic on `--hyb570514c` remained untouched. A further Azure execution requires a new explicit approval because the authorized retry has been consumed. |
| 2026-09-08 | D-036 500-PDF candidate populated; promotion blocked by one retrieval probe | After explicit approval for one additional execution, immutable image `sha256:638cdeae203dcfcb8ffb5764de4b53c2aa2245f5c305d3da32956ed7188378eb` combined commits `64b268f` and `7b6123e`. Execution `job-corpus-batch-500-001-4n1id9a` revalidated the 483 existing originals without re-uploading their bytes, processed 4,514 bounded embedding inputs into 4,467 original-chunk vectors, uploaded the candidate Search documents, and reached exact manifest parity. Azure metrics recorded 2,647,203 input/processed tokens and 312 model requests in the execution window. Seven of eight final checks passed: five of six distributed source probes, internal-scope visibility, and the unauthorized-scope negative test. `source-probe-246` used the derived query `legend shower provision location entrance rating`; five results cleared the semantic threshold but none were the expected single-page drawing `GSA-06-ZZ-L00-DR-RAU-AR-1000 - L00 - Ground Floor Plan.pdf` (`src-779696c36d01b083fc855b9c`), so the job correctly failed closed and the candidate remains non-promotable pending a retrieval-policy decision. No further execution was attempted. Embedding capacity was immediately restored from 100 to 1 kTPM and verified. The disposable job, image tag/manifest/repository, and repository-scoped cleanup assignment `8b9c63c8-ce57-40ef-809b-4c8174c66f4f` were deleted and independently verified absent. The 483 candidate Blobs and populated isolated index remain for review. Live `consult-demo-v2`, its app setting, and 100% traffic on `--hyb570514c` remained untouched. |
| 2026-09-09 | D-037 candidate ranking diagnosed and all accepted content independently checked | A single bounded query-only job directly found both expected chunks for `src-779696c36d01b083fc855b9c`. The keyword-stuffed probe ranked the source eighth semantically and lexically; three natural questions ranked it first semantically (lexical ranks 10, 3, and 6). Ten comparable floor-plan drawings all ranked first for natural title-derived requests, so the evidence supports accepting D-036 as a probe-wording issue rather than an ingestion or drawing-type retrieval gap. No embedding, Search write, promotion, or live-index action occurred. Separately, an independent PDFium 5.13.0 pass compared every accepted original with overlap-deduplicated primary extraction text: all 483 documents and all 2,984 pages passed, with zero meaningful shortfalls, page-count mismatches, or hash mismatches; the lowest qualifying character coverage was 98.38%. Full evidence is recorded in `docs/candidate-ranking-and-completeness-diagnostic-2026-09-09.md`. Live traffic remains 100% on `--hyb570514c`/`consult-demo-v2`; the disposable job was deleted. |
| 2026-09-09 | D-038 quarantine register, realistic probe, and next size gate | Recorded all seventeen non-accepted sources without retrying them: fifteen exceeded the standing 2 GiB extraction ceiling and two produced no extractable text; none was password-protected or proven permanently unusable. The exact filenames, source IDs, peaks, and fresh-export recommendations are in `docs/corpus-batch-500-quarantine-and-next-size-2026-09-09.md`. Replaced the source-specific `source-probe-246` keyword derivation for `src-779696c36d01b083fc855b9c` with the validated natural question and added a regression test while preserving the deterministic fallback for all other sources. Combined real evidence now covers 600 attempts/583 accepted PDFs, 4,855 pages, 7,098 chunks, and 4,184,494 embedding-input tokens. The next proposed gate is 900 of the 1,179 presently runnable untouched PDFs (76.3%); time and cost remain intentionally unstated pending separate size approval. No Azure ingestion, promotion, index write, live change, or quarantine retry occurred. |
| 2026-09-09 | D-039 ranking-diagnostic ACR cleanup and 917-attempt estimate gate | After explicit approval, assigned `Container Registry Repository Contributor` temporarily to the operator at the existing ACR scope with ABAC condition restricted to repository `helmonic-consult-ranking-diagnostic`. Deleted only that repository, manifest `sha256:98874b85b5b9c7f034d0793f2eac1529e3e44afea517e8ee265ded3a7a49c128`, and tag `diag-1ddb4925`, then immediately revoked assignment `93910246-a8f8-4a99-a308-8710889e2dba`. Independent post-cleanup checks returned the repository absent and zero matching temporary role assignments. A separate live read-back confirmed 100% traffic still targets `--hyb570514c` and its configured index remains `consult-demo-v2`. The owner approved the scope of a subsequent 917-attempt PDF run (900 untouched PDFs plus the seventeen prior quarantines), with a 4 GiB per-document ceiling only for the fifteen former 2 GiB memory failures; the two no-text-layer sources remain non-OCR attempts expected to quarantine honestly. Time, cost, and current-host concurrency remain a separate unapproved gate, so no new payload processing, Blob write, embedding, Search write, index, job, image, capacity, promotion, or live change has started. |
| 2026-09-09 | D-040 917-attempt execution gate approved and exclusive lane proven locally | The owner approved one 917-attempt execution under a fifteen-hour wall-clock ceiling and USD 2 incremental ceiling: 900 deterministic untouched PDFs, all fifteen former memory failures under a batch-only 4 GiB/ten-minute limit, and both no-text sources without OCR. The coordinator now applies the elevated limit only to the fifteen named sources, drains every other worker before one starts, requires at least 3 GiB available physical memory plus 5 GiB available commit, and retains the ordinary 2 GiB limit and parent-owned atomic checkpoints for every other source. Eighteen local tests passed, including real two-reader extraction, checkpointing, bounded memory termination, exclusive 4 GiB dispatch, and both physical/commit gates. A selection-only proof returned exactly 900 untouched plus seventeen retries, with zero overlap against the 583 previously accepted sources. No source PDF was opened and no Azure data/resource change occurred during this proof; the real local run is the next action. |
| 2026-09-09 | D-041 917-attempt launcher array-binding correction | The first operator launch stopped in under one second before context creation or source access because a nested native `powershell.exe` invocation flattened the two exclusion payload paths and treated the second as an invalid positional argument. The launcher now invokes the signed script in-process with named parameter splatting, preserving both exclusion paths and the retry-summary array as typed collections. The abandoned-context check found no 917 context or document progress, all eighteen corpus hardening tests still pass, and no PDF read, Azure call, cost, index write, or live change occurred. |
| 2026-09-11 | D-042 917-attempt recovery reconciled; isolated candidate populated with one probe failure | The final local reconciliation accounts for all 917 attempts: 903 accepted (717 direct and 186 repair-verified, including 22 recovered by the bounded follow-up) and 14 quarantined (eleven genuine recovery failures and three no-text sources). Independent verification found zero duplicate source IDs, matched every source hash, page count, integrity outcome, citation namespace, and all 12,589 chunk IDs/hashes, and rewrote the parent checkpoint to the exact 903/14 terminal state. A candidate-only schema preflight created `consult-candidate-batch-917-v1`; its temporary Search role and job were then independently verified absent. The one approved standing-identity execution `job-corpus-batch-917-001-xwkkb81` ran for 57m44s, completed Blob verification, 7,038,684 embedding input tokens across 897 model requests, Search writes, and exact 12,589-chunk manifest parity before the final retrieval gate. Seven of eight checks passed. Only `source-probe-1` failed: the deterministic fallback query `living laundry corridor parcel accessible cleaner's` retained five results but not its expected floor-plan source `src-008d7b77069971cd95cd1405`. Exact manifest parity proves the source is present, so this is not an ingestion omission; no claim is made yet about its realistic-query ranking, and no retry or promotion occurred. Embedding capacity was restored to 1 kTPM. The disposable job, both temporary image manifests/tags, repository, and repository-scoped cleanup role were deleted and independently verified absent. Candidate Blobs and the candidate index were retained for diagnosis. Live configuration remains `consult-demo-v2` with 100% traffic on `--hyb570514c`. The running accepted-document total is 1,486 of the approximately 2,064 target, leaving approximately 293 PDFs plus 285 Word documents pending the Word conversion-fidelity gate. |
| 2026-09-11 | D-043 drawing-probe generation corrected at the source | The second drawing-only false failure exposed a general defect: the deterministic fallback treated spatially extracted legends and room labels as if their first six distinct words formed a useful query. The generator now detects structured `Drawing Title` metadata across plain and Markdown-table extraction, joins multiline titles, includes `Project Title` when available, and asks a natural title-derived document-finding question. Regression coverage includes the previously validated ground-floor-plan override, the D-042 multiline Goatstown acoustic drawing, a second generic drawing, and the unchanged non-drawing fallback. Drawing failures remain fail-closed and require diagnosis; there is no blanket 7/8 waiver. This metadata-first probe rule is standing policy for all future batches. No Azure resource, candidate data, live index, or traffic was changed. |
